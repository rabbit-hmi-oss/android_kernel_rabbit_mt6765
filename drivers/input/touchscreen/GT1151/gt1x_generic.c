...
static u8 gt1151_cfg_fhdp_amoled[] = {
...
s32 gt1x_init_debug_node(void)
{
	...
	GTP_INFO("create_proc_entry %s SUCCESS.", GT1X_DEBUG_PROC_FILE);
	return 0;
}
...
s32 gt1x_init_panel(void)
{
	...
	if (!strcmp(tpd_dts_data.cfg_version, "config16")) {
		cfgs[0] = gt1151_cfg16;
		cfg_lens[0] = CFG_GROUP_LEN(gt1151_cfg16);
	} else if (!strcmp(tpd_dts_data.cfg_version, "config17")) {
		cfgs[0] = gt1151_cfg17;
		cfg_lens[0] = CFG_GROUP_LEN(gt1151_cfg17);
	} else if (!strcmp(tpd_dts_data.cfg_version, "config18")) {
		cfgs[0] = gt1151_cfg18;
		...
		GTP_ERROR("cfg %s get failed, please check dts!\n",
...
void gt1x_generic_exit(void)
{
	remove_proc_entry(GT1X_DEBUG_PROC_FILE, NULL);
}
EXPORT_SYMBOL_GPL(gt1x_generic_exit);
