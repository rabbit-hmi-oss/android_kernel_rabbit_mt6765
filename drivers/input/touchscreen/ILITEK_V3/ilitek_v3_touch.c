...
extern unsigned int ili_gesture_cfg;
...
//Antai <AI_BSP_TP> <hehl> <2021-08-09> copy 1900 tp code begin
ssize_t ilitek_gesture_buf_show(struct device *dev,struct device_attribute *attr, char *buf);
ssize_t ilitek_gesture_buf_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count);
...
struct ilitek_gesture_st {
    struct ges_feature_info f_point;
};

static struct ilitek_gesture_st ilitek_gesture_data;

//Antai <AI_BSP_TP> <wutj> <2022-03-10> add finger cartong location start
void set_location_area_point()
{
		ilitek_gesture_data.f_point.start_x = 60;
		ilitek_gesture_data.f_point.start_y = 500;
		ilitek_gesture_data.f_point.end_x = 660;
		ilitek_gesture_data.f_point.end_y = 1100;
		ilitek_gesture_data.f_point.width = 600;
		ilitek_gesture_data.f_point.height = 600;
		ilitek_gesture_data.f_point.mid_x = 360;
		ilitek_gesture_data.f_point.mid_y = 800;
		ilitek_gesture_data.f_point.top_x = 360;
		ilitek_gesture_data.f_point.top_y = 500;
		ilitek_gesture_data.f_point.bottom_x = 360;
		ilitek_gesture_data.f_point.bottom_y = 1100;
		ilitek_gesture_data.f_point.left_x = 60;
		ilitek_gesture_data.f_point.left_y = 800;
		ilitek_gesture_data.f_point.right_x = 660;
		ilitek_gesture_data.f_point.right_y = 800;
}
//Antai <AI_BSP_TP> <wutj> <2022-03-10> add finger cartong location end
...
ssize_t ilitek_gesture_buf_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count = sizeof(ilitek_gesture_data.f_point)/sizeof(ilitek_gesture_data.f_point.data[0]);

    for (i = 0; i < count; i++)
    {
        if(i==count-1)
            len += sprintf(buf+len,"%d",ilitek_gesture_data.f_point.data[i]);
        else
            len += sprintf(buf+len,"%d,",ilitek_gesture_data.f_point.data[i]);
    }
...
//Antai <AI_BSP_TP> <hehl> <2021-08-09> copy 1900 tp code begin
	if((gc->code == GESTURE_DOUBLECLICK) && (ilitek_wake_switch == 1)){
	...
	if(ilitek_gesture_switch == 1){
		...
		case GESTURE_V_LEFT:
            ...
            if(TST_BIT(ili_gesture_cfg, TYPE_V_LEFT)) {
                gc->gcode = KEY_GESTURE_U_LEFT;
                ILI_INFO("[report position]gesture '>'.\n");
            }			
			gc->clockwise = 1;
			break;				
		case GESTURE_RIGHT:
		...
		case GESTURE_V_DOWN:
            ...
            if(TST_BIT(ili_gesture_cfg, TYPE_V_DOWN)) {
                gc->gcode = KEY_GESTURE_U_DOWN;
                ILI_INFO("[report position]gesture '^'.\n");
            }			
			gc->clockwise = 1;
			break;			
		case GESTURE_O:
			gc->type  = GESTURE_O;				
			if(TST_BIT(ili_gesture_cfg, TYPE_O)) {
				gc->gcode  = KEY_GESTURE_O;
			}				
		if (ilits->rib.nReportResolutionMode == POSITION_LOW_RESOLUTION) {
	...
	case GESTURE_W:
		gc->type  = GESTURE_W;
			if(TST_BIT(ili_gesture_cfg, TYPE_W)) {
				gc->gcode  = KEY_GESTURE_W;
			}
		gc->clockwise = 1;
		break;
	case GESTURE_M:
		gc->type  = GESTURE_M;
			if(TST_BIT(ili_gesture_cfg, TYPE_M)) {
				gc->gcode  = KEY_GESTURE_M;
			}
		gc->clockwise = 1;
		break;
	case GESTURE_V:
		gc->type  = GESTURE_V;
			if(TST_BIT(ili_gesture_cfg, TYPE_V_UP)) {
				gc->gcode  = KEY_GESTURE_V;
			}
		gc->clockwise = 1;
		break;
	case GESTURE_C:
		gc->type  = GESTURE_C;
			if(TST_BIT(ili_gesture_cfg, TYPE_C)) {
				gc->gcode  = KEY_GESTURE_C;
			}
		gc->clockwise = 1;
		break;
	case GESTURE_E:
		gc->type  = GESTURE_E;
			if(TST_BIT(ili_gesture_cfg, TYPE_E)) {
				gc->gcode  = KEY_GESTURE_E;
			}
		gc->clockwise = 1;
		break;
	case GESTURE_S:
		gc->type  = GESTURE_S;
			if(TST_BIT(ili_gesture_cfg, TYPE_S)) {
				gc->gcode  = KEY_GESTURE_S;
			}
		gc->clockwise = 1;
		break;
	case GESTURE_Z:
		gc->type  = GESTURE_Z;
			if(TST_BIT(ili_gesture_cfg, TYPE_Z)) {
				gc->gcode  = KEY_GESTURE_Z;
			}
		gc->clockwise = 1;
		break;
	...
	default:
		...
		if(gc->gcode != 0) {
			input_report_key(input, gc->gcode, 1);
			input_sync(input);
			input_report_key(input, gc->gcode, 0);
			input_sync(input);
		}
	}
//Antai <AI_BSP_TP> <hehl> <2021-08-09> copy 1900 tp code end	
