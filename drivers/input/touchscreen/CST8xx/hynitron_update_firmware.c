...
struct hynitron_fw_array hynitron_fw_grp[20] = {
	...
	{ "capacitive_hynitron_cst8xx_update",   cst8xx_fw,   0x10,0x01, CST836, (sizeof(cst8xx_fw))},
...
struct hynitron_chip_type_array hynitron_chip_type_grp[50] = {
    ...
    {CST836   ,HYN_CHIP_CST8XX  ,  HYN_CHIP_PRODUCT_LINE_SEL_CAP, 0x6A},
...
int cst8xx_firmware_info(struct i2c_client * client);
...
static int cst78xx_update(u16 startAddr,u16 len,u8* src){
    ...
    if (cst78xx_enter_bootmode() == -1){
		HYN_INFO("cst78xx_enter_bootmode failed!\n");
       return -1;
    }
    sum_len = 0;
...
int cst8xx_boot_update_fw(struct i2c_client *mclient){
	...
	HYN_INFO("cst78xx_enter_bootmode %d \n", cst78xx_enter_bootmode());
    if (cst78xx_enter_bootmode() == 0){ 
        ...
        if((hyn_ts_data->fw_length) > 10){
			...
			HYN_INFO("cst78xx_read_checksum %d \n", chipchecksum);
