...
{
{mtk6737,Kernel 3.18,android 7.1.2},
{mtk8167,Kernel 3.18,android 8.0},
...
extern int hynitron_touch_setmode(struct i2c_client *client,int mode,int isMult);
/*****************************************************************************
...
*  Return:
*****************************************************************************/
static irqreturn_t hyn_eint_interrupt_handler(int irq,  void *data)
{
    ...
    if (node)
    {
		...
		if(hyn_ts_data->use_irq ==0){
			...
			return -1;
				...
				ret = request_irq(hyn_ts_data->use_irq, (irq_handler_t)hyn_eint_interrupt_handler,
		        hyn_ts_data->pdata->irq_gpio_flags, "Hynitron Touch Int", NULL);   	
		        if (ret == 0 ){
...
KEY_PROCESS:
    ...
    if(!hyn_ts_data->pdata->report_key_position){
		if(key_status&0x80) {
	        ...
	        if((key_status&0x7F)==0x03) {
				if((i==hyn_ts_data->key_index)||(hyn_ts_data->key_index==0xFF)) {
					...
					HYN_INFO(" cst3xx key up code:%d,key id:%d. \n", hyn_ts_data->pdata->key_code[hyn_ts_data->key_index], hyn_ts_data->key_index);
		...
		if(key_status&0x80) {
	        ...
	        if((key_status&0x7F)==0x03) {
				if((i==hyn_ts_data->key_index)||(hyn_ts_data->key_index==0xFF)) {
					HYN_INFO(" cst3xx key down x:%d,y id:%d. \n",hyn_ts_data->pdata->key_x_coords[i], hyn_ts_data->pdata->key_y_coords[i]);
...
int hyn_input_dev_int(struct hynitron_ts_data *ts_data)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, 480,0, 0);  // fuzz --flat
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, 640,0, 0);
	...
	thread = kthread_run(hyn_touch_handler, 0, HYN_DRIVER_NAME);
	if (IS_ERR(thread)) {
		ret = PTR_ERR(thread);
...
void hyn_ts_data_init(struct i2c_client *client)
{
	...
	if (ret < 0){
		hyn_ts_data->config_chip_type		=HYN_CHIP_TYPE_CONFIG_BACK;
		hyn_ts_data->config_chip_product_line =HYN_CHIP_PRODUCT_LINE_MUT_CAP;				
	
		hyn_ts_data->chip_ic_main_addr     =HYN_MAIN_IIC_ADDR_CONFIG_BACK;	
		hyn_ts_data->pdata->x_overturn     =HYN_X_REVERT;
		hyn_ts_data->pdata->y_overturn     =HYN_Y_REVERT;
		hyn_ts_data->pdata->xy_exchange	   =HYN_XY_EXCHANGE;
		hyn_ts_data->work_mode 			   =HYN_WORK_MODE_NORMAL;	
		...
		if(scan_id==ARRAY_SIZE(hynitron_chip_type_grp))
		{
			...
			hyn_ts_data->chip_ic_main_addr=HYN_MAIN_IIC_ADDR_CONFIG_BACK;
			client->addr						=HYN_MAIN_IIC_ADDR_CONFIG_BACK;	
			HYN_DEBUG("client addr is different,now use default addr,please confirm. client->addr=0x%x, default iic addr=0x%x.\r\n",client->addr,HYN_MAIN_IIC_ADDR_CONFIG_BACK);
	}
...
extern void och1970_suspend(void);
extern void och1970_resume(void);
/*****************************************************************************
*  Name: hyn_suspend
...
*  Return:
*****************************************************************************/
static void hyn_suspend(struct device *h)
{
    HYN_FUNC_ENTER();
...
*  Return:
*****************************************************************************/
static void hyn_resume(struct device *h)
{
	HYN_FUNC_ENTER();
...
static int hyn_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	HYN_INFO(" hyn Device Tree get regulator :vtouch ! \n");
	...
	if (ret != 0)
		HYN_ERROR("Failed to enable regulator vtouch : %d\n", ret);
...
err_end:
   ...
   //if point is null,may cause dump
	if(hyn_ts_data->use_irq!=0){
		...
		hyn_ts_data->use_irq =0;
	}
	if(hyn_ts_data->pdata!=NULL)
	kfree(hyn_ts_data->pdata);
	if(hyn_ts_data!=NULL)
	kfree(hyn_ts_data);
    HYN_FUNC_EXIT();
    return ret;
...
static int hyn_remove(struct i2c_client *client)
{
	...
	if (gpio_is_valid(hyn_ts_data->pdata->reset_gpio))
		gpio_free(hyn_ts_data->pdata->reset_gpio);
	if (gpio_is_valid(hyn_ts_data->pdata->irq_gpio))
		gpio_free(hyn_ts_data->pdata->irq_gpio);
	if(hyn_ts_data->use_irq!=0){
		//free_irq(hyn_ts_data->use_irq,NULL);
		hyn_ts_data->use_irq =0;
	}
...
*  Return:
*****************************************************************************/
static int  hynitron_driver_init(void)
{
	int ret=-1;
	HYN_FUNC_ENTER();
...
*  Return:
*****************************************************************************/
static void hynitron_driver_exit(void)
{
    HYN_FUNC_ENTER();
...
static int hynitron_mtk_local_init(void)
{
	HYN_FUNC_ENTER();
	if(hynitron_driver_init()!= 0){
		HYN_ERROR(" hynitron_driver_init FAIL.\n");
		return -1;
	}
	if(tpd_load_status == 0)  {
		hynitron_driver_exit();
		return -1;
	}	
	/*set vendor string*/
	...
	HYN_INFO("  hyn is end %s, %d\n", __FUNCTION__, __LINE__);
    HYN_FUNC_EXIT();
	
	return 0;
}

static struct tpd_driver_t hynitron_mtk_ts_device = {
	.tpd_device_name =HYN_DRIVER_NAME,
	.tpd_local_init = hynitron_mtk_local_init,
	.suspend =hyn_suspend,
	.resume = hyn_resume,
};

/* called when loaded into kernel */
static int __init hynitron_mtk_ts_init(void)
{
	HYN_INFO("HYN_DRIVER_VERSION: %s.\n", HYN_DRIVER_VERSION);
	HYN_FUNC_ENTER();
	
	tpd_get_dts_info();
	if (tpd_driver_add(&hynitron_mtk_ts_device) < 0)
		HYN_ERROR("add hynitron_mtk_ts_device failed.\n");
...
static void __exit hynitron_mtk_ts_exit(void)
{
	HYN_FUNC_ENTER();
	tpd_driver_remove(&hynitron_mtk_ts_device);
	HYN_FUNC_EXIT();
}

module_init(hynitron_mtk_ts_init);
module_exit(hynitron_mtk_ts_exit);
