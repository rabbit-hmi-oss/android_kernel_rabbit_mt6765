...
static struct imgsensor_info_struct imgsensor_info = {
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,//SENSOR_OUTPUT_FORMAT_RAW_R,//SENSOR_OUTPUT_FORMAT_RAW_B,//SENSOR_OUTPUT_FORMAT_RAW_Gr
    .mclk = 24,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
    { 8160, 6144,    0,    0, 8160, 6144, 4080, 3072,  0,   0, 4080, 3072,    0,    0, 4080, 3072}, /* preview sync with capture */
    { 8160, 6144,    0,    0, 8160, 6144, 4080, 3072,  0,   0, 4080, 3072,    0,    0, 4080, 3072}, /* capture */
    { 8160, 6144,    0,    0, 8160, 6144, 4080, 3072,  0,   0, 4080, 3072,    0,    0, 4080, 3072}, /* normal video sync with capture */
    { 8160, 6144,    0,    0, 8160, 6144, 2040, 1536,  0,   0, 2040, 1536,    0,    0, 2040, 1536}, /* high  speed video */
    { 8160, 6144,    0,    0, 8160, 6144, 2040, 1536,  0,   0, 2040, 1536,    0,    0, 2040, 1536}, /* slim video */
...
static void write_shutter(kal_uint32 shutter)
{
    ...
    if (shutter > 65530) {
        ...
        exposure_time = (shutter + 64) * 127 / 15062500; // shutter * imgsensor.line_length / imgsensor.pclk;
        long_shutter = (shutter + 64) / 63; // exposure_time * 0x05DB;
...
static void slim_video_setting(void)
{
    ...
    check_output_stream_off();
    // Antaiui <AI_BSP_CAM> <xieht> <2021-06-04> add check streaming off end
   // table_write_cmos_sensor(addr_data_pair_2000x1500_30fps,sizeof(addr_data_pair_2000x1500_30fps) / sizeof(kal_uint16));
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001));
            printk("s5kjn1mipi_Sensor read out sensor id 0x%x\n", *sensor_id);
	//	if (*sensor_id == 0x38E1)
		//	{
			//	*sensor_id = 0xF8D1;
		//	}
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("s5kjn1mipi_Sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                chipID = read_cmos_sensor_16_8(0x000F);
                LOG_INF("chipID: %d\n", chipID);
                return ERROR_NONE;
            }
            printk("s5kjn1mipi_Sensor Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
    }
   // if ((*sensor_id != imgsensor_info.sensor_id)&&(*sensor_id != 0xF8D1)) {
   if (*sensor_id !=  imgsensor_info.sensor_id) {
...
static kal_uint32 open(void)
{
   ...
   // if ((imgsensor_info.sensor_id != sensor_id)&&(0xF8D1 != sensor_id)) 
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_CUSTOM1:
            ...
            imgsensor.min_frame_length = imgsensor.frame_length;
            spin_unlock(&imgsensor_drv_lock);
            LOG_INF("calc custom1 frame_length: %d, dummy_line = %d, f_frame_length = %d, f_min_frame_length = %d\n", frame_length, imgsensor.dummy_line, imgsensor.frame_length, imgsensor.min_frame_length);
            set_dummy();
            break;
        case MSDK_SCENARIO_ID_CUSTOM2:
            ...
            imgsensor.min_frame_length = imgsensor.frame_length;
            spin_unlock(&imgsensor_drv_lock);
            LOG_INF("calc custom2 frame_length: %d, dummy_line = %d, f_frame_length = %d, f_min_frame_length = %d\n", frame_length, imgsensor.dummy_line, imgsensor.frame_length, imgsensor.min_frame_length);
            set_dummy();
            break;
        default:
            frame_length = imgsensor_info.pre.pclk / framerate * 10 / imgsensor_info.pre.linelength;
...
static void hdr_write_tri_shutter(kal_uint16 le, kal_uint16 me, kal_uint16 se)
{
    ...
    if (imgsensor.autoflicker_en) {
        realtime_fps = imgsensor.pclk / imgsensor.line_length * 10 / imgsensor.frame_length;
        if (realtime_fps >= 257 && realtime_fps <= 305)
            set_max_framerate(256, 0);
        else if (realtime_fps >= 147 && realtime_fps <= 110)
            set_max_framerate(146, 0);
        else
            write_cmos_sensor_16_16(0x0340, imgsensor.frame_length & 0xFFFF);
    } else {
...
static void hdr_write_tri_gain(kal_uint16 lg, kal_uint16 mg, kal_uint16 sg)
{
    ...
    if (global_gain < BASEGAIN || global_gain > 16 * BASEGAIN) {
        ...
        else if (global_gain > 64 * BASEGAIN)
            global_gain = 64 * BASEGAIN;
    }
